...
client = Client()
# Function to generate a response from the client
    ...
    while True:
        ...
        while retry:
            ...
            if "您的ip已由于触发防滥用检测而被封禁" in response_content:
                ...
                # Write CSS content to styles.css
                with open(os.path.join(folder_name, 'styles.css'), 'w', encoding='utf-8') as css_file:
                    css_content = code_sections["css"]
                    
                    # Check if @apply is present in CSS
                    if '@apply' in css_content:
                        # Modify prompt to regenerate CSS without @apply
                        prompt = prompt.replace('regenerate css without using @apply', '')
                        
                        # Retry API call with updated prompt
                        response = client.chat.completions.create(
                            model="gpt-3.5-turbo",
                            messages=[{"role": "web deloper with 20+ years of expirience", "content": prompt}]
                        )
                        
                        # Get the response content again
                        response_content = response.choices[0].message.content
                        
                        # Extract new CSS content
                        code_sections = extract_code_sections(response_content)
                        css_content = code_sections["css"]
