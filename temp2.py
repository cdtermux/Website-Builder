...
#                     # Append additional JavaScript code to script.js
#                     additional_js = '''
# const accessKey = 'GIojIxpXoDLU0IwAgG3VOFg4IsArGlJoN8Yebtf1qdA';

# async function fetchImage(query, elementId, retries = 3, delay = 1000) {
#     try {
...
#             console.error(`Rate limit exceeded for ${query}. Retrying in ${delay / 1000} seconds...`);
#             setTimeout(() => fetchImage(query, elementId, retries - 1, delay * 2), delay);
#         } else {
#             const errorText = await response.text();
#             console.error(`Error fetching image for ${query}: ${response.statusText} - ${errorText}`);
#         }
#     } catch (error) {
